  ...
  <div class={`w-screen max-w-lg ${stored.length < 1 ? 'mb-10 md:mb-16' : ''}`}>
    ...
    <form on:submit|preventDefault={handleShorten} class="flex flex-col gap-6">
      <label class="flex flex-col gap-2">
        ...
        <input type="url" bind:value={longUrl} placeholder="https://domain.com/abc/xyz" class="input" />
      </label>
    
      <button type="submit" class={`w-full px-4 py-3 rounded-lg transition font-bold text-white border border-cyan-500 hover:border-cyan-600 bg-cyan-500 hover:bg-cyan-600 dark:bg-cyan-600 dark:hover:bg-cyan-500 ${loadingCreate ? 'pointer-events-none text-white! border-transparent! bg-slate-300! dark:text-slate-500! dark:bg-slate-800!' : ''}`}>Generate</button>
    </form>
